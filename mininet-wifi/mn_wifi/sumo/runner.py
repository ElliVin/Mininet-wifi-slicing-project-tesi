...
MNWIFI_SUMO = os.path.abspath(os.path.join(BASE_DIR, ".."))


import sys, csv
#sys.path.insert(0, "/home/eventura/Progetti/mininet-wifi/mininet-wifi/mn_wifi")
...
#prendo la mi aversione di mobility
from my_mobility import Mobility
import inspect
print("*** mobility.py in uso:", inspect.getfile(Mobility), flush=True)
...
class sumo(Mobility):
    ...
    def start(self, cars, config_file, clients, port,
        ...
        if config_file == '':
            sumoConfig = os.path.join(os.path.dirname(__file__), "MAPPA/mappa.sumocfg")
        else:
