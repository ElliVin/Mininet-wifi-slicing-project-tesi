"""
   VERSIONE PERSONALIZZATA DI mobility.py PER L'ESECUZIONE DELLA SIMULAZONE
"""
...
class Mobility(object):
    ...
    #AGGIUNTA 20/10/25---------------------------------------------
    def ap_to_host_ip(self,ap_node):
        """
        ap1 -> 10.0.0.201, ap2 -> 10.0.0.202, ... ap10 -> 10.0.0.210
        """
        m = re.search(r'(\d+)$', ap_node.name)
        idx = int(m.group(1)) if m else 1
        return f"10.0.0.{200 + idx}"
    #--------------------------------------------------------------

    #AGGIUNTA 26/10/25---------------------------------------------
    def host_ip_to_ap(self, ip_addr):
        """
        Inverso di ap_to_host_ip():
        10.0.0.201 -> ap1, 10.0.0.202 -> ap2, ... 10.0.0.210 -> ap10
        """
        try:
            # Estrae l'ultima parte dell'indirizzo IP
            last_octet = int(str(ip_addr).strip().split('.')[-1])
            # Calcola indice AP (201 -> 1, 202 -> 2, ecc.)
            idx = last_octet - 200
            if idx < 1:
                raise ValueError("IP non valido per AP host")
            return f"ap{idx}"
        except Exception:
            return "ap_unk"  # fallback in caso di errore
    #-------------------------------------------------------------
    
    #AGGIUNTA: file di pulizia (.log e processi zombie) -----------
    
    def cleanup_iperf_files(self, net):
        """
        Pulisce eventuali PID e log vecchi di iperf
        """
        for sta in net.stations:
            pid_file = f"/tmp/{sta.name}.pid"
            log_file = f"/tmp/{sta.name}.log"
        
            if os.path.exists(pid_file):
                os.remove(pid_file)
                print(f"*** [cleanup] removed {pid_file}")
            if os.path.exists(log_file):
                os.remove(log_file)
                print(f"*** [cleanup] removed {log_file}")

    #--------------------------------------------------------------
    
    #AGGIUNTA 20/10/25 --------------------------------------------
    def monitor_ping_and_start_iperf(self, sta, target):
        """
        Pinga 'target' in un thread separato e avvia iperf quando riceve risposta.
        Evita duplicati e pulisce i processi.
        """
        log_file = f"/tmp/{sta.name}_ping.log"

        # se c'era un ping precedente, chiudilo
        try:
            if hasattr(sta, '_ping_proc') and sta._ping_proc and sta._ping_proc.poll() is None:
                sta._ping_proc.terminate()
        except Exception:
            pass

        sta.current_target = target
        sta.ping_thread_started = True
        ...
        try:
            ...
            for line in iter(ping_proc.stdout.readline, ''):
                ...
                # se risponde, avvia iperf
                if "64 bytes from" in line and not getattr(sta, 'iperf_running', False):
                    # chiudi il ping: non serve più
                    try:
                        if ping_proc and ping_proc.poll() is None:
                            ping_proc.terminate()
                    except Exception:
                        pass
                    ...
                    print(f"*** [ping-check] {sta.name} connessa a {target}! Avvio iperf...")

                    # se c'era un iperf precedente, killalo (prudenza)
                    try:
                        pid = sta.cmd(f"cat /tmp/{sta.name}.pid").strip()
                        if pid:
                            sta.cmd(f"kill -9 {pid}")
                    except Exception:
                        pass

                    sta.iperf_running = True

                    #AGGIUNTA 26/10/25 ----------------------------------------
                    #aggiungo la data di inizi sul file di salvataggio staN_apM
                    ap_name = self.host_ip_to_ap(target)
                    # assicura che la cartella esista
                    #os.makedirs("/home/eventura/Progetti/mininet-wifi/mininet-wifi/LOG_STAZIONI", exist_ok=True)
                    #save_file = f"/home/eventura/Progetti/mininet-wifi/mininet-wifi/LOG_STAZIONI/{sta.name}_{ap_name}.log"
                    os.makedirs(LOG_DIR, exist_ok=True)
                    save_file = os.path.join(LOG_DIR, f"{sta.name}_{ap_name}.log")

                    sta.cmd(f"echo 'ORARIO DI INIZIO: {start_time}' >> {save_file}")
                    #----------------------------------------------------------
                    sta.cmd(f"iperf -c {target} -t 9999 -i 2 > /tmp/{sta.name}_iperf.log 2>&1 & echo $! > /tmp/{sta.name}.pid")

                    # SOSTITUIRE CON QUESTA RIGA SE SI VOGLIONO VEDERE GLI IPERF: 
                    #sta.cmd(f"xterm -hold -e 'iperf -c {target} -t 9999 -i 2 | tee /tmp/{sta.name}_iperf.log' & echo $! > /tmp/{sta.name}.pid")
            ...
            print(f"*** [ping-check] Errore su {sta.name}: {e}")
    ...
    def ap_out_of_range(self, intf, ap_intf):
        ...
        if ap_intf == intf.associatedTo:
            ...
            #AGGIUNTA: kill dell'iperf--------------------------------
            sta = intf.node
            if hasattr(sta, 'iperf_running') and sta.iperf_running:
                try:
                    pid = sta.cmd(f"cat /tmp/{sta.name}.pid").strip()
                    if pid:
                        sta.cmd(f"kill -9 {pid}")
                    sta.iperf_running = False
                    print(f"*** [iperf] {sta.name} stopped iperf")
                    # timestamp fine iperf ---------------------------
                    end_time = datetime.now().strftime("%H:%M:%S")
                    sta.end_time = end_time
                    #-------------------------------------------------
                    #AGGIUNTA 26/10/25----------------------------------------
                    log_tmp = f"/tmp/{sta.name}_iperf.log"
                    # assicura che la cartella esista
                    #os.makedirs("/home/eventura/Progetti/mininet-wifi/mininet-wifi/LOG_STAZIONI", exist_ok=True)
                    #save_file = f"/home/eventura/Progetti/mininet-wifi/mininet-wifi/LOG_STAZIONI/{sta.name}_{ap_intf.node.name}.log"
                    os.makedirs(LOG_DIR, exist_ok=True)
                    save_file = os.path.join(LOG_DIR, f"{sta.name}_{ap_intf.node.name}.log")
                    
                    # Se il log temporaneo esiste, aggiungilo al file permanente
                    if os.path.exists(log_tmp):
                        sta.cmd(f"cat {log_tmp} >> {save_file}")

                    # Poi aggiungi l’orario di fine sessione
                    sta.cmd(f"echo 'ORARIO DI FINE: {end_time}\n' >> {save_file}")
                    #---------------------------------------------------------
                except Exception as e:
                    print(f"*** [iperf] error stopping {sta.name}: {e}")
            ...
            sta.ping_thread_started = False
            sta.current_target = None
            #---------------------------------------------------------
    ...
    def ap_in_range(self, intf, ap, dist):
        if intf.node.__class__.__name__.lower() == 'sumo':
            return
        for ap_intf in ap.wintfs.values():
            if isinstance(ap_intf, master):
                ...
                #AGGIUNTA 20/10/25 -----------------------------------------
                # 🔹 DEBUG: ingresso nel range
                now = datetime.now().strftime("%H:%M:%S")
#                print(f"[{now}] {intf.node.name} a {dist:.2f}m da {ap_intf.node.name} (RSSI={rssi:.1f} dBm)")

                if ap_intf == intf.associatedTo:
#                    print(f"[{now}]  {intf.node.name} si è associato ad {ap_intf.node.name}")
                    if intf not in ap_intf.associatedStations:
                        ap_intf.associatedStations.append(intf)

                    sta = intf.node
                    target = self.ap_to_host_ip(ap_intf.node)  # 10.0.0.201..210 in base all'AP
#                    print(f"[{now}] {sta.name} proverà a pingare {target} (host di {ap_intf.node.name})")
                    # avvia il monitor ping → iperf solo se non già avviato per questo target
                    if getattr(sta, 'current_target', None) != target:
                        # se stava parlando con altro target, chiudi ping e iperf correnti
                        try:
                            if hasattr(sta, '_ping_proc') and sta._ping_proc and sta._ping_proc.poll() is None:
                                sta._ping_proc.terminate()
                        except Exception:
                            pass
                        try:
                            if getattr(sta, 'iperf_running', False):
                                pid = sta.cmd(f"cat /tmp/{sta.name}.pid").strip()
                                if pid:
                                    sta.cmd(f"kill -9 {pid}")
                                sta.iperf_running = False
                        ...
                        sta.ping_thread_started = False
                        # avvio nuovo thread verso il giusto hn
                        thread(target=self.monitor_ping_and_start_iperf, args=(sta, target)).start()

                #-----------------------------------------------------------

                #NB: per riportare come prima elimina questo blocco e decommenta quello sotto 
